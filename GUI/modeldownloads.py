...
import webview
from nicegui import ui, ElementFilter, app
...
def GFS_Load(download_button, top_entry, bottom_entry, left_entry, right_entry, generated_elements, content):
    ...
    async def Download(run_date, run_time):
        ...
        def confirm_selection():
            if int(to_hour.value) > int(from_hour.value):
                ...
                download_thread = (threading.Thread(
                    target=lambda: GFS_download(hours=selected_hours, run_date=run_date.replace('gfs.', ''),
                                               run_time=run_time,
                                               filename=file,
                                               levels=selected_levels, variables=selected_vars, subregion=subregion)
                                              ,daemon=True))
                ...
                with dialog, ui.card().style('padding: 20px;'):
                    ...
                    panoply_path = os.getenv("PANOPLY_PATH")
                    grib_file = os.path.abspath(file)

                    ui.button("Open With Panoply", on_click=lambda: (
                    dialog.close(), subprocess.run(f'"{panoply_path}" "{grib_file}"', shell=True)))
        ...
        with (ui.dialog() as dialog, ui.card().style('padding: 20px;')):
            ...
            ui.button('Submit', on_click=lambda: (ui.notify("Download Has Finished"), confirm_selection())
